...
export default function Diretos() {
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        title="Vendas em Direto"
        subtitle={`Gestão de pedidos capturados durante live sessions (${diretosOrders.length})`}
        filters={
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-[hsl(340_72%_45%)] outline-none transition-all font-bold text-xs"
            />
          </div>
        }
      />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          {diretosOrders.length === 0 && (
            <div className="col-span-full">
              <EmptyState
                icon={<Video className="w-10 h-10 text-slate-300" />}
                title="Nenhuma venda de direto pendente"
                description='As vendas marcadas como "Direto" no POS aparecerão aqui'
              />
            </div>
          )}
        </div>
