...
export default function POS() {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex flex-col bg-slate-50/50 dark:bg-slate-900/40 backdrop-blur-md rounded-[2.5rem] border border-slate-200/60 dark:border-white/10 overflow-hidden shadow-2xl">
                <div className="p-3 border-b border-slate-100 dark:border-white/5 space-y-3">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                        <div className="flex-1 max-w-xl relative group">
                            ...
                            <input
                                type="text"
                                placeholder="Ref / Nome / Instagram (V2)..."
                                value={searchTerm}
                ...
                <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
                    {viewMode === 'list' && filteredProducts.length > 0 && (
                        <div className="px-5 py-2 mb-2 grid grid-cols-[32px_2fr_1fr_1fr_1fr] items-center gap-4 border-b border-slate-100 dark:border-white/5 sticky top-0 bg-white dark:bg-slate-900 z-10">
                            <div className="w-8 h-8 opacity-0" />
                            <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Artigo / Ref</span>
                            <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest text-center">STK</span>
                            <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest text-center">Custo</span>
                            <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest text-right">PVP</span>
                        </div>
                    )}
            ...
            <motion.div 
                animate={{ width: isCartCollapsed ? '60px' : '360px' }}
            ...
            >
                ...
                <div className="px-3 py-3 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 space-y-2">
                    ...
                    <div className="bg-primary p-1.5 rounded-lg flex items-center justify-between shadow-lg shadow-primary/20">
                        <span className="font-black text-white uppercase tracking-tighter text-[7.5px]">TOTAL V2</span>
                        <span className="font-black text-xs text-white">{formatCurrency(cartTotal)}</span>
                    </div>
                    ...
                    <button
                        ...
                        className="w-full py-1.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 disabled:opacity-20 font-black text-[8.5px] uppercase tracking-widest rounded-lg transition-all flex items-center justify-center gap-1"
                    >
                        ...
                        >
                            ...
                            {checkoutStep === 3 && (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                    ...
                                    <div className="flex gap-4 pt-4">
                                        <button onClick={() => setCheckoutStep(2)} className="flex-1 py-4 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-400 font-black text-[10px] uppercase tracking-widest rounded-2xl transition-all">Anterior</button>
                                        <button
                                            disabled={!paymentMethod || isProcessing}
                                            onClick={handleFinalizeSale}
                                            className="flex-2 flex-[2] py-4 bg-emerald-500 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-lg shadow-emerald-500/20 hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                                        >
