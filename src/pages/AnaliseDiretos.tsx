...
export default function AnaliseDiretos() {
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        title="Análise de Diretos"
        subtitle="Performance consolidada das Live Sessions"
      />
      ...
      <div className="flex-1 overflow-y-auto p-5 bg-[hsl(38_25%_96%)] space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* Main Chart */}
          <div className="lg:col-span-2 bg-white rounded-[14px] border border-slate-100 p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-sm font-black text-slate-900 uppercase tracking-tight">Performance por Sessão</h2>
          ...
          {/* Top Products */}
          <div className="bg-white rounded-[14px] border border-slate-100 p-6 flex flex-col">
            <div className="mb-5">
            ...
            <div className="flex-1 space-y-4">
              {topProducts.slice(0, 5).map((prod, i) => (
                <div key={prod.ref} className="flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center font-black text-[10px] text-rose-500 shrink-0 border border-slate-100">
                      {i + 1}
                    </div>
                    <div className="min-w-0">
                      <p className="text-[11px] font-black text-slate-900 uppercase truncate">{prod.name}</p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{prod.quantity} Vendidos</p>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-[11px] font-black text-slate-900">{formatCurrency(prod.revenue)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Live Ranking Table */}
          <div className="bg-white rounded-[14px] border border-slate-100 p-6 overflow-hidden">
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                    <th className="pb-3">Sessão</th>
                    <th className="pb-3">Vendas</th>
                    <th className="pb-3 text-right">Faturação</th>
                    <th className="pb-3 text-right">Efeito</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {sessions.slice(0, 10).map((session) => (
                    <tr key={session.id} className="hover:bg-slate-50 transition-colors">
                      <td className="py-3">
                        <p className="text-[11px] font-black text-slate-900 uppercase truncate max-w-[200px]">
                          {session.name}
                        </p>
                      ...
                      <td className="py-3">
                        <span className="px-2 py-1 bg-rose-50 text-rose-500 text-[10px] font-black rounded-lg">
                          {session.orders} Pedidos
                        </span>
                      </td>
                      <td className="py-3 text-right text-[11px] font-black text-slate-900">
          ...
          {/* Top Live Customers */}
          <div className="bg-white rounded-[14px] border border-slate-100 p-6">
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-5 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {topCustomers.slice(0, 4).map((customer, i) => (
                <div key={customer.instagram || i} className="p-4 bg-slate-50 rounded-[10px] border border-slate-100 hover:border-rose-500/20 transition-all">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-[10px] font-black text-slate-900 uppercase truncate">{customer.name}</p>
