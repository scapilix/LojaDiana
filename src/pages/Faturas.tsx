...
export default function Faturas() {
    ...
    <div className="flex items-center gap-2">
      ...
      {isFiltered && (
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white rounded-[10px] text-xs font-black uppercase tracking-widest transition-all"
        >
          <X className="w-4 h-4" />
        </button>
      )}
      <select
        ...
        className="px-3 py-1.5 bg-white border border-[hsl(35_18%_90%)] rounded-[10px] font-bold text-xs focus:outline-none"
      >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="pl-8 pr-3 py-1.5 bg-white border border-[hsl(35_18%_90%)] rounded-[10px] w-48 focus:outline-none text-xs font-bold"
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        title="Faturas"
        filters={faturasFilters}
        actions={
          <button
            onClick={() => setIsFormOpen(true)}
            className="bg-[hsl(340_72%_45%)] text-white text-[12px] font-bold px-3 py-1.5 rounded-[10px] hover:bg-[hsl(340_72%_38%)] transition-colors"
          >
            + Nova Fatura
          </button>
        }
      ...
      <div className="flex-1 overflow-y-auto p-5 bg-[hsl(38_25%_96%)] space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          {/* Table card */}
          <div className="lg:col-span-2 bg-white rounded-[14px] border border-[hsl(35_18%_90%)] shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
            <div className="px-5 py-3.5 border-b border-[hsl(35_18%_92%)] flex justify-between items-center">
              <h2 className="text-[13px] font-bold text-[hsl(20_15%_8%)]">Registos Recentes</h2>
              <span className="text-[11px] text-[hsl(30_8%_55%)]">{filteredFaturas.length} encontrados</span>
            </div>

            <div className="overflow-x-auto">
              {filteredFaturas.length === 0 ? (
                <EmptyState title="Sem faturas" description="Adicione a primeira fatura" />
              ) : (
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-[hsl(35_18%_92%)]">
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3">Data</th>
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3">Tipo</th>
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3 text-center">Status</th>
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3 text-center">Anexos</th>
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3">Entidade</th>
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3">Categoria</th>
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3 text-right">Valor Total</th>
                      <th className="text-left text-[9px] font-bold text-[hsl(30_8%_55%)] uppercase tracking-[0.07em] px-4 py-3 text-center">Ações</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredFaturas.map((fatura: Fatura) => (
                      <tr key={fatura.id} className="border-b border-[hsl(35_18%_96%)] hover:bg-[hsl(38_25%_98%)] transition-colors group">
                        <td className="px-4 py-3 text-[12px] font-bold text-slate-600">
                          {new Date(fatura.data).toLocaleDateString('pt-PT')}
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${fatura.tipo_fatura === 'Compras' ? 'bg-blue-50 text-blue-600' :
                              fatura.tipo_fatura === 'Vendas' ? 'bg-emerald-50 text-emerald-600' :
                        ...
                        <td className="px-4 py-3 text-center">
                          <StatusBadge status={fatura.fatura_status ?? 'C/F'} />
                        </td>
                        <td className="px-4 py-3 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            {fatura.fatura_url ? (
                              ...
                              <span className="p-1.5 opacity-20"><FileText className="w-3.5 h-3.5" /></span>
                            )}
                            {fatura.comprovativo_url ? (
                              ...
                              <span className="p-1.5 opacity-20"><FileImage className="w-3.5 h-3.5" /></span>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="text-[12px] font-black text-slate-900 uppercase tracking-tight">
                              {fatura.entidade}
                              {fatura.fornecedor && (
                                <span className="ml-1.5 text-slate-400 font-bold text-[10px] normal-case">({fatura.fornecedor})</span>
                              )}
                            </span>
                            <span className="text-[10px] text-slate-400 font-bold">{fatura.numero_fatura || 'S/ Nº'}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-right text-[13px] font-black text-[hsl(20_15%_8%)]">
                  ...
                  <tfoot>
                    <tr className="bg-[hsl(38_25%_97%)] border-t-2 border-[hsl(35_18%_88%)]">
                      <td colSpan={6} className="px-4 py-3 text-[11px] font-bold text-[hsl(30_8%_55%)]">Total</td>
                      <td className="px-4 py-3 text-[13px] font-black text-[hsl(20_15%_8%)] text-right">{formatCurrency(totalSpent)}</td>
                      <td />
                    </tr>
                  </tfoot>
                </table>
          ...
          {/* Chart card */}
          <div className="bg-white rounded-[14px] border border-[hsl(35_18%_90%)] shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-5">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-8 h-8 rounded-[10px] bg-purple-50 flex items-center justify-center">
                <TrendingUp className="w-4 h-4 text-purple-600" />
              </div>
              <div>
                <h4 className="text-[13px] font-bold text-[hsl(20_15%_8%)]">Por Categoria</h4>
                <p className="text-[10px] text-[hsl(30_8%_55%)]">Distribuição de gastos</p>
              </div>
            </div>
            <div className="h-56">
