...
export default function ValesTroca() {
  ...
  const voucherStatusMap = {
    active:  { bg: '#ecfdf5', text: '#059669', label: 'Ativo' },
    used:    { bg: '#eff6ff', text: '#2563eb', label: 'Usado' },
    expired: { bg: '#fef2f2', text: '#dc2626', label: 'Expirado' },
  };

  return (
    <div className="flex flex-col h-full bg-[hsl(38_25%_96%)]">
      <PageHeader title="Vales de Troca" subtitle="Gestão de vales e trocas de produtos" />
      ...
      <div className="bg-white dark:bg-white/5 rounded-[2.5rem] border border-slate-100 dark:border-white/10 shadow-xl overflow-hidden">
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col items-center gap-1">
                          <StatusBadge status={voucherStatusMap[status]?.label ?? status} map={Object.fromEntries(Object.values(voucherStatusMap).map(v => [v.label, v]))} />
                          {status === 'used' && voucher.used_at && (
                            <span className="text-[8px] text-slate-400">{formatDate(voucher.used_at)}</span>
                          )}
                        </div>
                      </td>
        ...
        {filteredVouchers.length === 0 && (
          <EmptyState
            ...
            description="Altere os filtros ou adicione uma nova troca para gerar um vale de troca."
          />
        )}
      </div>
      </div>
